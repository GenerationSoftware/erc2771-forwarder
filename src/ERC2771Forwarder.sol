...
contract ERC2771Forwarder is EIP712, Nonces {
    ...
    function validate(ForwardRequestData calldata request) public view virtual returns (bool isTrustedForwarder, bool active, bool signerMatch) {
    ...
    function execute(ForwardRequestData calldata request) public payable virtual {
        ...
        (bool success, bytes memory revertMessage) = _execute(request, true);
        if (!success) {
    ...
    /**
     * @dev Returns the hash of the request data.
     *
     * This hash can be used together with {ECDSA-recover} to obtain the signer of a message. For example:
     *
     * ```solidity
     * bytes32 digest = sigHash(request);
     * address signer = ECDSA.recover(digest, request.signature);
     * ```
     */
    function sigHash(
        ForwardRequestData calldata request, uint256 nonce
    ...
    function _recoverForwardRequestSigner(
        ...
        (address recovered, ECDSA.RecoverError err, ) = data.tryRecover(request.signature);
